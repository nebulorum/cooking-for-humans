...
object IterateeStep7 {
    ...
    new Consumer[I, List[T]] {
      def consume(input: Input[I]): ConsumerState[I, List[T]] =
        step.consume(input) match {
          case Error(e) if groupStart == step && accumulated.nonEmpty => Done (accumulated.reverse, input)
          case Error(e) => Error(e)
  ...
  def main(args: Array[String]) {
    ...
    println(readNestedMessage.consumeAll(nestedMessage.take(3)))
  }
